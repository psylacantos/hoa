...
public class ApproveRequest extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String button1 = request.getParameter("accept");
        String button2 = request.getParameter("reject");
        UserDAO uDao = new UserDAO();
        String uName = request.getParameter("uName_hd");
        if(button1 !=null){
            uDao.acceptRequest(uName);
        }else{
            uDao.rejectRequest(uName);
        }
